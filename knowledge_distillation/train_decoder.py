...
def main():
    ...
    parser.add_argument('--train-features', type=pathlib.Path,
                        default='/home/huangsiyong/ali/aokvqa/features/rationale_336px_train', required=False,
                        dest='train_features')
    parser.add_argument('--val-features', type=pathlib.Path,
                        default='/home/huangsiyong/ali/aokvqa/features/rationale_336px_val', required=False,
                        dest='val_features')
...
class LinearClassifier(pl.LightningModule):
    def __init__(self, objective, backbone, clip_model_type, inputs, vocab_len, lr=0.001, epochs=100,
        ...
        self.visual_linear = nn.Linear(out_dim, text_dim)
        self.linear0 = nn.Linear(text_dim, out_dim)
    ...
    def image_text_attn(self, image_feature, text_feature, text_mask):
        text_feature = self.text_linear(text_feature)
        item = image_feature.unsqueeze(2) + text_feature.unsqueeze(1)
        S_logit = self.dim2weight(torch.tanh(item)).squeeze(-1)
        text_mask = text_mask.unsqueeze(1)
        S_logit = S_logit.masked_fill(text_mask == 0, -1e30)
        return S_logit

    # def forward(self, encoder_input, encoder_mask, image_feature):
    #     text_feature = self.encoder(encoder_input, encoder_mask).last_hidden_state
    #
    #     encoder_v = self.visual_linear(image_feature)
    #     image_0 = encoder_v.unsqueeze(1)
    #
    #     attention_mask = torch.cat((encoder_mask[:, 0].unsqueeze(1).repeat(1, 3), encoder_mask), dim=1)
    #
    #     attn_logit = self.image_text_attn(image_0, text_feature, encoder_mask)
    #     image_text_attn, text_image_attn = attn_logit.softmax(-1), attn_logit.softmax(1)
    #     image_1 = torch.matmul(image_text_attn, text_feature)
    #     image_2 = image_0 * image_1
    #     encoder_outputs = torch.cat((image_0, image_1, image_2, text_feature), dim=1)
    #
    #     batch_size = image_feature.size(0)
    #     effective_batch_size = batch_size
    #     num_beams = 1
    #     decoder_start_token_id = 2  # 0?
    #     max_length = 20
    ...
    #         # input_ids = shift_tokens_right(input_ids, 1, 2)
    ...
    #         )
    #         next_token_logits = self.lm_head(outputs[0][:, -1, :])
    ...
    def forward_decode(self, image_feature, encoder_input, encoder_mask, decoder_input, decoder_mask):
        ...
        input_ids = shift_tokens_right(decoder_input, 1, 2)  # 0?
        outputs = self.decoder(
            input_ids=input_ids,
            attention_mask=decoder_mask,
            encoder_hidden_states=encoder_outputs,
    ...
    def loss_func3(self, t, t_norm):
        ...
        t2t = ((t_norm @ t.T) / self.temperature).softmax(dim=-1)
    ...
    def loss_func4(self, i, t_norm, gt_after):
        ...
        t2i = ((t_gt @ i.T) / self.temperature).softmax(dim=-1)
    ...
    def compute_loss(self, batch):
        i, t, gt, ids, mask, encoder_input, encoder_mask, decoder_input, decoder_mask = batch
        ...
        i2t = ((i @ t_norm.T) / self.temperature).softmax(dim=-1)
